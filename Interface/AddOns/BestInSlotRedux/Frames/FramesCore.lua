...
--	if self.db.char.options.showBiSTooltip and (self.db.char.options.tooltipCombat or not UnitAffectingCombat("player")) then
    ...
    if link then
      ...
      local exists, existType = BestInSlot:ItemExists(itemId)
      ...
      if itemId then
        ...
        if bisInfo and type(bisInfo) == "table" then
          ...
          if isBiS then
            ...
            if BestInSlot:HasItem(itemId, instanceID, true) then
          ...
          if item.multiplesources then
            ...
            for dungeonname,v in pairs(item.multiplesources) do
              for bossid in pairs(v) do
                tooltip:AddLine(("%s%s %s(%s)|r"):format(BestInSlot.colorHighlight, BestInSlot:GetDescription(BestInSlot.BOSS, dungeonname, bossid), BestInSlot.colorNormal, dungeonname))
              end
            end
